...
async function asset_price_in_five_days() {
    ...
    for (let assetId of assetIds) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            let price = await api.rpc.swap.drylySellTokens(assetId, 1n * 10n ** 18n, blockHash);
            console.log(`current price is ${price.toHuman()}`);
            let originValue = assetId2PriceList.get(assetId) || [];
            assetId2PriceList.set(assetId, [(0, util_1.formatBalance)(price.toHuman(), { withUnit: 'AD3', decimals: 18 }), ...originValue]);
        }
    }
...
async function state_call_account_nounce() {
    ...
    const [gid, gau, non] = await Promise.all([
        ...
        api.call.accountNonceApi.accountNonce('EQBwtmKWCyRrQ8yGWg7LkB8p7hpEKXZz4qUg9WR8hZmieCM')
    ]);
    console.log('setId=', gid.toHuman(), 'nonce=', non.toHuman(), 'authorities=', gau.toHuman());
}
async function loadLinkEntries() {
    let apiWs = await createApi();
    console.log("start loading all entries");
    const PAGE_SIZE = 50;
    let last_key = "0xeff9e899844ad32fcaf1efb3ce8e842bc6af308cbe71b047400b4b0d341ead7a002521041a1b644d654bf451642f1d3f7889176cd195eca478deb3f087";
    for (;;) {
        const new_entries = await apiWs.query.linker.linksOf.entriesPaged({ args: [], pageSize: PAGE_SIZE, startKey: last_key });
